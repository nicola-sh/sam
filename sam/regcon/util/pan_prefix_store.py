...
from sam.regcon.util.pan_prefix_text import (
    DEFAULT_PREFIX_LEN,
    load_prefixes_from_text,
    prefixes_to_text,
)
from sam.regcon.util.app_paths import pan_prefix_path
...
def _migrate_from_legacy_plaintext(path: Path, prefix_len: int) -> list[str]:
    ...
    if path.is_file():
        try:
            ...
            pass
    from sam.regcon.config.settings import default_config_path, load_config
